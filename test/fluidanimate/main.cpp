...
void InitSim(char const *fileName, unsigned int threadnum) {
	...
	fptype particleMass = 0.5 * doubleRestDensity
			/ (restParticlesPerMeter * restParticlesPerMeter
	...
	std::cout << "Grids steps over x, y, z: " << delta.x << " " << delta.y
			<< " " << delta.z << std::endl;
	...
	int rv0 = posix_memalign((void **) (&cells), CACHELINE_SIZE,
			sizeof(struct Cell) * numCells);
	int rv1 = posix_memalign((void **) (&cells2), CACHELINE_SIZE,
			sizeof(struct Cell) * numCells);
	int rv2 = posix_memalign((void **) (&cnumPars), CACHELINE_SIZE,
			sizeof(int) * numCells);
	int rv3 = posix_memalign((void **) (&cnumPars2), CACHELINE_SIZE,
			sizeof(int) * numCells);
	int rv4 = posix_memalign((void **) (&last_cells), CACHELINE_SIZE,
			sizeof(struct Cell *) * numCells);
...
void RebuildGridMT_T(void **args) {
	OTM_BEGIN();
	...
	*cell = OTM_SHARED_READ_P(last_cells[index]);
	*np = OTM_SHARED_READ_I(cnumPars[index]);
	int numPars= OTM_SHARED_READ_I(cnumPars[index]);
	if ((*np % PARTICLES_PER_CELL == 0)
			&& (numPars != 0)) {
		OTM_SHARED_WRITE_P((*cell)->next, cellpool_getcell(&pools[tid]));
		OTM_SHARED_WRITE_P(*cell, (*cell)->next);
		OTM_SHARED_WRITE_P(last_cells[index], *cell);
	}
		...
		for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
			for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
				...
				for (int j = 0; j < np2; ++j) {
					...
					int ci = (int) ((cell2->p[j % PARTICLES_PER_CELL].x
							- domainMin.x) / delta.x);
					int cj = (int) ((cell2->p[j % PARTICLES_PER_CELL].y
							- domainMin.y) / delta.y);
					int ck = (int) ((cell2->p[j % PARTICLES_PER_CELL].z
							- domainMin.z) / delta.z);
					...
					// Critical Section, shared reads and writes happens here
					//
					//
#ifdef pthreads
					if (border[index])
					...
					if (cell == NULL) {
						INFO("NULL POINTER EXCEPTION");
					}

					cell->p[np % PARTICLES_PER_CELL] = cell2->p[j
							% PARTICLES_PER_CELL];
					cell->hv[np % PARTICLES_PER_CELL] = cell2->hv[j
							% PARTICLES_PER_CELL];
					cell->v[np % PARTICLES_PER_CELL] = cell2->v[j
							% PARTICLES_PER_CELL];
...
void InitDensitiesAndForcesMT(int tid) {
	...
	INFO("Start TX:" << age);
	O_API::run_in_order(InitDensitiesAndForcesMT_T, NULL, age);*/
	for (int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
			for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
				for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
					int index = (iz * ny + iy) * nx + ix;
...
////////////////////////////////////////////////////////////////////////////////
void ComputeDensitiesMT_T(void **args) {
	OTM_BEGIN();
	Cell *cell = (Cell*)args[0];
	int index = *((int*)args[1]);
	fptype tc = *((fptype*)args[2]);
	OTM_SHARED_WRITE_P(cell->density[index], OTM_SHARED_READ_P(cell->density[index]) + tc);
	OTM_END();
}
						...
						for (int iparNeigh = 0; iparNeigh < numNeighPars;
							...
							if (&neigh->p[iparNeigh % PARTICLES_PER_CELL]
								...
								fptype distSq =
										(cell->p[ipar % PARTICLES_PER_CELL]
												- neigh->p[iparNeigh
														% PARTICLES_PER_CELL]).GetLengthSq();
								if (distSq < hSq) {
									fptype t = hSq - distSq;
									fptype tc = t * t * t;
#ifdef pthreads
									...
									if (border[index]) {
										int index = ipar % PARTICLES_PER_CELL;
										void **args = new void*[3];
										args[0] = (void*)cell;
										args[1] = (void*)&index;
										args[2] = (void*)&tc;
									...
									if (border[indexNeigh]) {
										int index = iparNeigh% PARTICLES_PER_CELL;
										void **args = new void*[3];
										args[0] = (void*)neigh;
										args[1] = (void*)&index;
										args[2] = (void*)&tc;
										...
										neigh->density[iparNeigh
												% PARTICLES_PER_CELL] += tc;
#endif

								}
							}
							//move pointer to next cell in list if end of array is reached
							if (iparNeigh % PARTICLES_PER_CELL
...
void ComputeForcesMT_T(void **args) {
    OTM_BEGIN();
...
void ProcessCollisionsMT(int tid) {
	...
	INFO("Start TX:" << age);
	O_API::run_in_order(ProcessCollisionsMT_T, NULL, age);*/
	for (int iz = grids[tid].sz; iz < grids[tid].ez; ++iz) {
		for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy) {
			for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
				...
				for (int j = 0; j < np; ++j) {
					...
					if (ix == 0) {
						fptype diff = parSize - (pos.x - domainMin.x);
						if (diff > epsilon)
							cell->a[ji].x += stiffnessCollisions * diff
									- damping * cell->v[ji].x;
					}
					if (ix == (nx - 1)) {
					...
					if (iy == 0) {
						...
						if (diff > epsilon)
							cell->a[ji].y += stiffnessCollisions * diff
									- damping * cell->v[ji].y;
					}
					if (iy == (ny - 1)) {
					...
					if (iz == 0) {
						...
						if (diff > epsilon)
							cell->a[ji].z += stiffnessCollisions * diff
									- damping * cell->v[ji].z;
					}
					if (iz == (nz - 1)) {
		...
		for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
			for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
				...
				for (int j = 0; j < np; ++j) {
					...
					cell->v[j % PARTICLES_PER_CELL] = cell->hv[j
							% PARTICLES_PER_CELL] + v_half;
...
int MAIN_FLUIDANIMATE(int argc, char *argv[]) {
	...
	thread_start(AdvanceFramesMT, (void*) &framenum);
