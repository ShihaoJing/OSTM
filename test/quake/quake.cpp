...
/*--------------------------------------------------------------------------*/ 
void transational_smvp(void **args) {
	OTM_BEGIN();
	double ***A = K;
...
static void smp_worker(void *args) {
	int tid = thread_getId();
	int workers = O_API::get_workers_count();
	void* arg[1];
	for (int i = tid; i < ARCHnodes; i += workers) {
		arg[0] = (void*)&i;
		O_API::run_in_order(transational_smvp, arg, i);
	}
...
void get_Enu(properties *prop, double *E, double *nu){
	double ratio;
...
void element_matrices(double vertices[][3], properties *prop, double Ke[][12], double Me[])
{
	double ds[3][4];
	double sum[3];
	double jacobian[3][3];
...
void abe_matrix(double vertices[][3], int bv[], properties *prop, double Ce[])
{
	int i, j;
	double area;
