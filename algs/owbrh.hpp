...
again:
		LOG("Commit " << age);
// make sure writeset are not locked
		{
			ListNode<EntryVersion<void**, void*>*>* itr = writeset.elements.header;
			while (itr != NULL) {
				EntryVersion<void**, void*>* bucket = itr->value;
				...
				for(int i=0; i<bucket_size; i++){
					...
					LOG("Writer1= " << lock->version);
				}
				itr = itr->next;
			}
		}
        LOG("HTM-Commit " << age);
// at this point the current Write-Write should be resolved
		...
		while(1){
			...
			if ((status & _XABORT_EXPLICIT) && _XABORT_CODE(status)==25){
                LOG("HTM-Status " << status);
				goto again;
			}
			tries--;
			if (tries <= 0) {
...
again2:
			...
			if(!complete && (validation==1 || last_completed > initial_last_completed) && !validateReadSet())
				return false;
		}
		htm_fail_count += HTM_RETRIES - tries;
