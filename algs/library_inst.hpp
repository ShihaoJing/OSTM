...
namespace commons
{
   ...
   */
  template <typename T, size_t S, typename X>
  struct DISPATCH
  {
      ...
      TM_INLINE
      static void write(T* addr, T val, X* thread)
      {
  ...
  template <typename X>
  struct DISPATCH<float, 4, X>
  {
      TM_INLINE
      static float read(float* addr, X* thread)
      {
      ...
      TM_INLINE
      static void write(float* addr, float val, X* thread)
      {
          union { float f;  void* v;  } v;
          v.f = val;
  ...
  template <typename X>
  struct DISPATCH<const float, 4, X>
  {
      TM_INLINE
      static float read(const float* addr, X* thread)
      {
          union { float f;  void* v;  } v;
          v.v = thread->read((void**)addr STM_MASK(~0x0));
          return v.f;
      ...
      TM_INLINE
      static void write(const float*, float, X*)
      {
          LOG("You should not be writing a const float!");
      }
  };

  /*** specialization for 8-byte types... need to do two reads/writes */
  template <typename T, typename X>
  struct DISPATCH<T, 8, X>
  {
      TM_INLINE
      static T read(T* addr, X* thread)
      {
   ...
   */
  template <typename X>
  struct DISPATCH<double, 8, X>
  {
      TM_INLINE
      static double read(double* addr, X* thread)
      {
          // get second word's address
          void** addr2 = (void**)((long)addr + 4);
          union {
      ...
      TM_INLINE
      static void write(double* addr, double val, X* thread)
      {
          // compute the two addresses
          void** addr1 = (void**) addr;
          void** addr2 = (void**) ((long)addr + 4);
  ...
  template <typename X>
  struct DISPATCH<const double, 8, X>
  {
      TM_INLINE
      static double read(const double* addr, X* thread)
      {
      ...
      TM_INLINE
      static void write(const double*, double, X*)
      {
          LOG("You should not be writing a const double!");
      }
  };
  ...
  template <typename X>
  struct DISPATCH<long double, 16, X>
  {
      TM_INLINE
      static double read(double* addr, X* thread)
      {
...
#error Cannot figure out the right dispatch mechanism
	...
	class AbstractTransaction{
		public:
	  	  	virtual void suicide(int reason) = 0;

			virtual void* read(void** so) = 0;
		  	TM_INLINE inline long read_l(long* so){
		  		long r = DISPATCH<long, sizeof(long), AbstractTransaction>::read(so, this);
		  		return r;
			}
			TM_INLINE inline int read_i(int* so) {
				return DISPATCH<int, sizeof(int), AbstractTransaction>::read(so, this);
			}
			TM_INLINE inline float read_f(float* so) {
				return DISPATCH<float, sizeof(float), AbstractTransaction>::read(so, this);
			}
		    template <typename T>
		    TM_INLINE inline T read_t(T* addr) {
			  return DISPATCH<T, sizeof(T), AbstractTransaction>::read(addr, this);
		    }

			virtual inline void write(void** so, void* value) = 0;
			TM_INLINE inline void write_l(long* so, long value){
				DISPATCH<long, sizeof(long), AbstractTransaction>::write(so, value, this);
			}
			TM_INLINE inline void write_i(int* so, int value){
				DISPATCH<int, sizeof(int), AbstractTransaction>::write(so, value, this);
			}
			TM_INLINE inline void write_f(float* so, float value){
				DISPATCH<float, sizeof(float), AbstractTransaction>::write(so, value, this);
			}
		    template <typename T>
		    TM_INLINE inline void write_t(T* addr, T val) {
			  DISPATCH<T, sizeof(T), AbstractTransaction>::write(addr, val, this);
		    }
	};
